...
public final class NetworkFrame {
    ...
    public final long ptsUs, ticket, documentRevision, viewportRevision;

    public NetworkFrame(byte[] value, int cw, int ch, int vw, int vh, long pts, long token, long document, long viewport) {
        ...
        if (pts < 0 || token < 1 || document < 0 || viewport < 0) throw new IllegalArgumentException("NETWORK_FRAME_IDENTITY");
        bytes = value.clone();
        codedWidth = cw; codedHeight = ch; visibleWidth = vw; visibleHeight = vh;
        ptsUs = pts; ticket = token;
        documentRevision = document; viewportRevision = viewport;
    }
