...
function Panel({ctx}: {ctx: Context}) {
  ...
  function request(command: ProbeCommand) {
    ...
    if (command.op === 'disconnect' || command.op === 'release' || command.op === 'takeover' || command.op === 'navigate' || command.op === 'stop') cancelComposition();
  ...
  <section className={network ? 'remote' : ''} data-state={snapshot.state} data-released={String(snapshot.released)} data-frames={snapshot.renderedFrames}>
    ...
    <section className="network" aria-label="远程浏览器 Host">
      <h2>远程浏览器</h2><p className="detail" role="status">{snapshot.networkConfigured === false ? '请先配置此设备的配对信息' : !connected ? '连接后默认观察，不影响 Agent 操作' : snapshot.connectionState === 'connecting' ? '正在连接…' : snapshot.controlMode === 'control' ? snapshot.inputReady ? '接管中 · 可导航、点击页面和输入文字' : '接管中 · 等待最新画面同步' : snapshot.controlMode === 'waiting' ? '等待当前原子操作完成，再交给你控制' : '观察模式 · Agent 可继续操作'}</p>
      ...
      <form className="address-entry" onSubmit={event => { event.preventDefault(); if (!canNavigate || url.length === 0) return; request({op:'navigate',epoch:snapshot.epoch!,url}); }}><input id="address" type="url" autoCorrect="off" spellCheck={false} aria-label="远程页面地址" value={url} onChange={event => setUrl(event.currentTarget.value)} placeholder="输入 https://、http:// 或 data:text/html 地址" disabled={!canNavigate} /><button id="navigate" type="submit" disabled={!canNavigate || url.length === 0}>打开</button></form>
